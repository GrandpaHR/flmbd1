...
const LAST_CHANNEL_KEY = "streamtv_last_channel";
...
let favorites = [];
try {
  const saved = localStorage.getItem("streamtv_favorites");
  if (saved) {
...
function setupVolumeControl() {
  ...
  // Always start muted for reliable autoplay on fresh page load
  video.muted = true;
  updateVolumeButtonState(true);

  video.addEventListener("volumechange", () => {
    updateVolumeButtonState(video.muted);
    localStorage.setItem("streamtv_muted", video.muted);
  });

  // Restore user's last mute preference after a short delay (once video is playing)
  setTimeout(() => {
    const wasMuted = localStorage.getItem("streamtv_muted");
    if (wasMuted === "false" && video.readyState >= 2) {
      video.muted = false;
    }
  }, 2000);
}
...
function toggleFavorite(event, url) {
  ...
  localStorage.setItem("streamtv_favorites", JSON.stringify(favorites));
    ...
    currentHls.on(Hls.Events.MANIFEST_PARSED, () => {
      video.play().then(() => {
        ...
        video.play().catch(() => {
          ...
          loader.querySelector("span").innerHTML = 'Click to play ▶';
          loader.querySelector("span").style.cursor = 'pointer';
          loader.querySelector("span").onclick = () => { loader.classList.add('hidden'); togglePlay(); };
          const spinner = loader.querySelector(".spinner");
...
function togglePiP() {
  ...
  if (!video.src && !video.srcObject) {
    alert("Play a channel first.");
    return;
  }

  if (video.readyState < 2) {
    alert("Wait for the video to load before entering PiP.");
    return;
  }

  video.requestPictureInPicture().catch(err => {
    console.warn("PiP error:", err);
    alert("Could not enter PiP. Try playing the channel first.");
  });
}
...
function showTermsModal(type, event) {
  ...
  const text = type === "privacy" 
    ? "At StreamTV, we value your privacy. We do not collect or store any personal data. All streams are sourced from third-party public playlists and played locally in your browser or application. Your preferences are saved only on your local device."
    : "Welcome to StreamTV! Our services are provided free of charge for streaming live channels. We do not host any of the video content; all streams are sourced from publicly available public playlists. By using this app, you agree to comply with your local copyright and streaming laws.";
...
function closeAppBanner() {
  ...
  if (banner) {
    ...
    localStorage.setItem("streamtv_hide_app_banner", "true");
  }
}
  ...
  fetch(configUrl)
    ...
    .then(data => {
      if (data && data.buildCode && data.buildCode > currentBuildCode) {
        ...
        const laterTime = localStorage.getItem("streamtv_update_later_time");
        const laterBuild = localStorage.getItem("streamtv_update_later_build");
...
function closeUpdateModal() {
  ...
  if (modal) {
    ...
    if (updateBuild) {
      localStorage.setItem("streamtv_update_later_time", Date.now().toString());
      localStorage.setItem("streamtv_update_later_build", updateBuild);
    }
  }
}
...
function checkDisclaimer() {
  const accepted = localStorage.getItem("streamtv_disclaimer_accepted");
  if (!accepted) {
...
function acceptDisclaimer() {
  localStorage.setItem("streamtv_disclaimer_accepted", "true");
...
function setupPictureInPicture() {
  ...
  video.addEventListener("leavepictureinpicture", () => {
    ...
    pipBtn.innerHTML = '<i class="fa-solid fa-image"></i>';
  });
